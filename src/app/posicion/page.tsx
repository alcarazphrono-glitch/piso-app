...
function PosicionContenido() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      setEvento(await obtenerEvento(pos.evento_id));
    });
  }, [posicionId, router]);

  // PISO Core, checkpoint 3-sep-2026: esto ya no decide nada en el
  // navegador -- solo llama a simular_resultado_posicion_demo(), que corre
  // en Postgres, verifica que la posición sea tuya, y escribe el ledger.
  ...
  if (!posicion || !evento) return <Screen><p className="pt-8 text-ink-soft">Cargando tu posición…</p></Screen>;
